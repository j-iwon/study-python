...
while True:
    ...
    # 2번을 선택했을때 (수정)
        ...
        # 사용자에게 수정 하고싶은 상품명을 입력받는다
        name = input(search_name_message_for_update)
        # name_list에 사용자가 입력한 상품명이 있다면,
        if name in name_list:
            # 새로운 이름과 새로운 가격을 동시에 입력받는다 (구분점은 공백)
            new_name, new_price = input(update_message).split()
            # 새로운 이름이 name_list에 없다면 기존 인덱스 위에 저장한다
            if new_name not in name_list:
                ...
                #새로운 이름이 name_list 에 존재한다면(중복일경우) 오류 메세지를 출력한다
    ...
    # 3번을 선택했을 경우(삭제)
        ...
        # 사용자에게 삭제하고싶은 상품명을 입력받는다
        name = input(delete_message)
        # 상품명이 name_list에 있는 경우,
        if name in name_list:
            # name_list의 가격을 삭제한 후 이름을 삭제한다
            index = name_list.index(name)
