...
반복문
    ...
    while : 조건식이 True일 때 반복, False일 때 종료                         # 몇 번 출력해야할지 모를때
        ...
        whlie 조건식:
            실행할 문장
list
    여러 개의 저장공간이 나열되어 있는 것.                              # * 참조연산자 <- 값을 읽어온다. *(l+2) -> ㅣ[2] -> ㅣ=[1,2,3] 로 간편하게 씀
...
list 선언
    ...
    list명 = [값1, 값2, ...]                                    # 직접 입력 : 어떤 값을 넣을지 미리 알고있는 경우
    list명 = [값]* 칸수                                         # 어떤 값을 넣을지는 모르지만 몇 개를 넣을지 알고있을때 ex) [0] * 10
    list명 = []                                                # 몇 칸을 넣을지도 모를 경우 만든 다음 밑에서 작성
    list명 = list(range(start, end, step)                      # 함수 넣기 처음부터 규칙성 있는 값을 넣을 때


list 길이
    len(list명)                                                # 값이 변경되어도 len이 알아서 계산해줌
...
클래스 - 반
    ...
    1. 타입이다.
        클래스 안에 선언된 변수와 메소드를 사용하고 싶다면, # 클래스 안에 선언된건 함수(X) 메소드(O)
...
클래스 선언
    class 클래스명:
        필드(변수, 메소드)           # 클래스 안에 선언되는 모든 것을 필드라고 부른다.

클래스의 필드를 사용
    1. 객체화(instance)             #클래스는 객체로 접근해야한다. 이를 객체화 시킨다고 한다.
...
#은행 하나당 고객 list , 모든 은행 고객은 2차원 list
#각 은행마다 계좌 중복 검사
